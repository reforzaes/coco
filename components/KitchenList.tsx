...
const KitchenList: React.FC<KitchenListProps> = ({ kitchens, incidents }) => {
  ...
  return (
    <div className="animate-fade-in space-y-8">
      <div className="bg-white p-6 md:p-8 rounded-[3rem] shadow-sm border">
        <input
          type="text"
          className="w-full border-2 border-gray-100 rounded-3xl p-5 md:p-6 font-bold text-gray-700 outline-none focus:border-emerald-500 transition-all text-lg shadow-inner"
        ...
        <div className="bg-white rounded-[3rem] border-2 border-emerald-100 shadow-xl overflow-hidden animate-in slide-in-from-top-4 duration-500">
          <div className="p-6 md:p-10">
            <div className="flex justify-between items-start mb-8 md:mb-10">
              ...
              <button onClick={() => setSelectedKitchenId(null)} className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 font-black text-2xl">×</button>
            </div>
              ...
              <div className="relative space-y-6 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-emerald-500 before:via-gray-100 before:to-transparent">
                {unifiedTimeline.map((entry, idx) => (
                  <div key={idx} className="relative flex items-start gap-4 md:gap-8 group">
                    ...
                    <div className="ml-12 flex-grow bg-white border border-gray-100 p-5 md:p-6 rounded-[2rem] shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 mb-3">
                        ...
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase self-start md:self-auto ${entry.statusAtTime === TaskStatus.COMPLETED ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                          {entry.statusAtTime}
                        </span>
                      </div>
      ...
      <div className="bg-white rounded-[3rem] border shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-[800px] md:min-w-full divide-y divide-gray-100">
                ...
                return (
                  <tr key={k.id} className={`transition-colors cursor-pointer group ${isSelected ? 'bg-emerald-50' : 'hover:bg-gray-50'}`} onClick={() => setSelectedKitchenId(k.id)}>
                    ...
                    <td className="px-8 py-5">
                      <div className="text-gray-800 uppercase whitespace-nowrap">{k.clientName}</div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="text-[10px] text-gray-400 flex flex-col gap-1 uppercase whitespace-nowrap">
