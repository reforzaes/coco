...
const FilteredListModal: React.FC<FilteredListModalProps> = ({ filter, allKitchens, allIncidents, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-2 md:p-4 z-[200] animate-in fade-in duration-300">
      <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl max-w-6xl w-full max-h-[95vh] flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tighter uppercase leading-none">
                {filter.label} 
                <span className="ml-2 md:ml-4 text-emerald-600 bg-emerald-100 px-3 py-1 rounded-full text-[8px] md:text-[10px] align-middle font-black tracking-widest">
                  {filter.type === 'seller' ? 'VENDEDOR' : 'INSTALADOR'}
                </span>
              </h2>
              <p className="text-[8px] md:text-[9px] text-gray-400 font-black uppercase tracking-[0.3em] mt-2">Base de Datos de Rendimiento Operativo</p>
            </div>
            
            <div className="flex gap-4 border-l-0 md:border-l-2 md:pl-6 border-gray-200">
              <div className="flex flex-col text-center">
                <span className="text-[8px] md:text-[9px] font-black text-gray-400 uppercase tracking-widest">Cocinas</span>
                <span className="text-lg md:text-xl font-black text-emerald-700">{stats.total}</span>
              </div>
              <div className="flex flex-col text-center">
                <span className="text-[8px] md:text-[9px] font-black text-gray-400 uppercase tracking-widest">Pendientes</span>
                <span className={`text-lg md:text-xl font-black ${stats.ratio > 0 ? 'text-red-600' : 'text-emerald-500'}`}>
                  {stats.activeCount} ({stats.ratio.toFixed(1)}%)
                </span>
              </div>
            </div>
          </div>
          <button onClick={onClose} className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full hover:bg-red-50 text-gray-300 hover:text-red-500 transition-all text-2xl md:text-3xl font-black">×</button>
        </div>
        
        <div className="p-4 md:p-10 overflow-y-auto space-y-8">
          <section>
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 mb-4 md:mb-6 flex items-center gap-3">
            ...
            <div className="border-2 border-gray-50 rounded-[2rem] overflow-hidden shadow-sm">
              <div className="overflow-x-auto">
                <table className="min-w-[500px] md:min-w-full divide-y divide-gray-50 text-sm">
                      ...
                      return (
                        <React.Fragment key={k.id}>
                          <tr className={`transition-colors ${isExpanded ? 'bg-emerald-50/50' : 'hover:bg-gray-50'}`}>
                            ...
                            <td className="px-8 py-5 text-gray-700 font-bold uppercase whitespace-nowrap">{k.clientName}</td>
                          ...
                          {isExpanded && (
                            <tr key={`detail-${k.id}`}>
                              <td colSpan={3} className="px-4 md:px-8 py-6 bg-emerald-50/30">
                                <div className="bg-white rounded-[2rem] p-4 md:p-6 border-2 border-emerald-100 shadow-inner animate-in slide-in-from-top-2 duration-300">
        ...
        <div className="p-6 md:p-8 border-t bg-gray-50 text-right">
          <button onClick={onClose} className="w-full md:w-auto bg-gray-900 text-white font-black px-12 md:px-16 py-4 rounded-2xl uppercase tracking-[0.2em] text-[10px] hover:bg-black transition-all shadow-xl">Cerrar</button>
        </div>
      </div>
    </div>
  );
